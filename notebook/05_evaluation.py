...
- When user input is irregular or missing details such as ingredients, ask clarifying questions before finalizing the recipe.
...
def generate_recipe_response(
    ...
    # if STILL empty → fallback (never crash)
    if retrieved_df.empty:
        fallback = generate_baseline_response(
            ingredients,
            max_cooking_time,
            cuisine,
            nutrition_goal,
...
def build_baseline_prompt(
    ...
    return f"""
You are a general recipe generator.
...
You need to strictly follow cooking time, cuisine, or nutrition.
You can add extra ingredients if needed.
...
mode = st.radio(
    "Choose system",
    ["Full system (RAG)", "Baseline (LLM only)"]
)
    ...
    if not ingredients.strip():
        ...
        with st.spinner("Cooking up your recipe idea..."):
            ...
            if mode == "Full system (RAG)":
                ...
                result = generate_baseline_response(
                ingredients=ingredients,
                max_cooking_time=max_time_value,
                cuisine=cuisine,
                nutrition_goal=nutrition_goal,
